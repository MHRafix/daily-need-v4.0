	...
	const onSubmit = async (values, { resetForm }) => {
		...
		if (values) {
			reqSender(
				...
				setToastType,
				setToastOn,
				'client_pannel_api/my_account/signin_api'
			);
		}
	};
	...
	const onSubmit = async (values, { resetForm }) => {
		...
		if (user_data) {
			reqSender(
				...
				setToastType,
				setToastOn,
				'client_pannel_api/my_account/signup_api'
			);
		}
	};
		...
		if (order_data) {
			try {
				...
				const { data } = await axios.post(
					'/api/client_pannel_api/checkout/place_order',
					order_data
				);
...
export const PaymentFormValidator = (clientSecret) => {
	...
	const onSubmit = async (values, { resetForm }) => {
		...
		if (intentError) {
			...
			const url = '/api/client_pannel_api/manage_orders/update_order';
			fetch(url, {
				method: 'POST',
				headers: {
	...
	const onSubmit = async (values, { resetForm }) => {
		...
		if (products_data) {
			reqSender(
				...
				setToastType,
				setToastOn,
				'admin_pannel_api/manage_products/add_limited_products'
			);
		}
	};
	...
	const onSubmit = async (values, { resetForm }) => {
		...
		if (review_data) {
			...
			reqSender(
				...
				setToastType,
				setToastOn,
				'client_pannel_api/manage_reviews/add_reviews'
			);
		}
	};
